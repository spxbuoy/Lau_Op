...
sudo apt update -y
sudo apt upgrade -y
sudo apt install -y figlet unzip curl wget lsof
...
typing_banner() {
    local text="$1"
    local color="$2"
    echo -e "${color}\e[1m"  # Set color and bold
    for (( i=0; i<${#text}; i++ )); do
        echo -ne "${text:i:1}"
        sleep 0.1
    done
...
if [[ "$OS_ID" == "ubuntu" ]]; then
    echo -e "${GREEN}  » Your OS Is Supported ( $OS_NAME )"
elif [[ "$OS_ID" == "debian" ]]; then
    echo -e "${GREEN}  » Your OS Is Supported ( $OS_NAME )"
else
    echo -e "${RED}[ERROR] Your OS Is Not Supported ( ${YELLOW}$OS_NAME${NC} )"
    exit 1
fi
...
while IFS=" " read -r a b; do
    case $a in
        "MemTotal:") mem_total=$((b / 1024)) ;;
        "Shmem:") mem_used=$((mem_used + b / 1024)) ;;
        "MemFree:"|"Buffers:"|"Cached:"|"SReclaimable:") mem_used=$((mem_used + b / 1024)) ;;
    esac
done < /proc/meminfo
...
function first_setup() {
    ...
    OS_ID=$(grep -w ID /etc/os-release | head -n1 | cut -d '=' -f2 | tr -d '"')
    OS_NAM=$(grep -w PRETTY_NAME /etc/os-release | head -n1 | cut -d '=' -f2 | tr -d '"')
...
function nginx_install() {
    ...
    if [[ "$OS_ID" == *"ubuntu"* ]]; then
        ...
        print_success "Setup nginx For OS Is $(grep -w PRETTY_NAME /etc/os-release | head -n1 | cut -d '=' -f2 | tr -d '"')"
        apt -y install nginx
    else
...
echo -e "\e[32mloading...\e[0m" 
clear
izinsc="https://raw.githubusercontent.com/spxbuoy/Lau_Op/main/register"
# USERNAME
rm -f /usr/bin/user
...
wget -q https://raw.githubusercontent.com/spxbuoy/Lau_Op/main/config/fv-tunnel && chmod +x fv-tunnel && ./fv-tunnel
...
function ins_Fail2ban() {
    ...
    # Handle DDOS Flate folder
    DDOS_DIR="/usr/local/ddos"

    if [ -d "$DDOS_DIR" ]; then
        echo "Old DDOS Flate directory found, deleting..."
        sudo rm -rf "$DDOS_DIR"
    fi
